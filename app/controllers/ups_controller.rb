...
class UpsController < ApplicationController
  ...
  # def send
  #   # params = {:order => {:origin => [ :country => 'US',
  #   #                                   :state => 'CA',
  #   #                                   :city => 'Beverly Hills',
  #   #                                   :zip => '90210'
  #   #                                 ],
  ...
  #   #                                     :postal_code => '98117'
  #   #                                     ],
  #   #                     :packages => [[100,[93,10],:cylinder => true]]
  #   #                   }
  #   #           }
  #   # redirect_to estimate_path(params)
  # end
  ...
  def extract_info(order)
    ...
    # info[:price] = rates.first.price
    # info[:delivery_date] = rate.first.delivery_date
    info
  end
  ...
  def estimate_params
    ...
    estimate_hash = {}
    # estimate_hash[:packages] = params[:packages].map {|package| Package.new(package)}
    # estimate_hash[:origin] = Location.new(params[:origin])
    # estimate_hash[:destination] = Location.new(params[:destination])

    estimate_hash[:packages]    = [Package.new( 100,                        # 100 grams
